...
export default class Helper {
  ...
  // returns true/false, ensuring that something has been returned..
  static isNotNullAndUndefined(obj, props = []) {
    ...
    console.log(!bIsNullorUndefined)
  ...
  static async makeFav(word, sense) {
    console.log('Word Added to Favourite!')
  ...
  static async getFavList(search) {
    ...
    try {
      ...
      if(favList.length > 0 && Helper.isNotNullAndUndefined(search) 
          && search.length > 0) {

        let filteredList = favList.filter(
          item => item.word.toUpperCase().includes(search.toUpperCase()));
        return filteredList;
      }
